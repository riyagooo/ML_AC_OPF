...
from pypower.api import loadcase, ext2int
...
from scripts.fix_matpower_file import fix_matpower_file

def load_case5_from_file(data_dir="data"):
    """
    Load case5 data directly from the raw MATPOWER file content.
    
    Args:
        data_dir: Directory containing data files
    
    Returns:
        MATPOWER case data dictionary
    """
    # Path to original file
    file_path = os.path.join(data_dir, "pglib_opf_case5.m")
    
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"MATPOWER file not found: {file_path}")
    
    # Parse the MATPOWER file manually
    print(f"Parsing MATPOWER file: {file_path}")
    
    # Create a case data dictionary based on the contents of pglib_opf_case5.m
    case_data = {
        'baseMVA': 100.0,
        'bus': [
            # Bus format: [bus_id, type, Pd, Qd, Gs, Bs, area, Vm, Va, baseKV, zone, Vmax, Vmin]
        ...
        'gen': [
            # Gen format: [bus, Pg, Qg, Qmax, Qmin, Vg, mBase, status, Pmax, Pmin]
        ...
        'gencost': [
            # Gencost format: [model, startup, shutdown, ncost, cost parameters...]
    ...
    print("Successfully created case5 data structure with real topology from MATPOWER file")
    print(f"Buses: {len(case_data['bus'])}")
    print(f"Branches: {len(case_data['branch'])}")
    print(f"Generators: {len(case_data['gen'])}")
...
def main():
    ...
    learning_rate = 0.001
    hidden_channels = 64  # Good hidden dimension
    num_layers = 3  # Good network depth
    ...
    # Load the case5 data directly from MATPOWER file content
    print(f"Loading case5 network data from MATPOWER file content...")
    case_data = load_case5_from_file(data_dir)
    ...
    # Use a subset of 1000 samples for faster testing
    if len(solutions_data) > 1000:
        solutions_data = solutions_data.sample(1000, random_state=42)
    ...
    def mean_absolute_percentage_error(outputs, targets):
        return 100 * torch.mean(torch.abs((targets - outputs) / (targets + 1e-8)))
