...
if num_samples < len(setpoints):
    print(f"Selecting {num_samples} random samples from the dataset")
    random_indices = random.sample(range(len(setpoints)), num_samples)
...
combined_data.to_csv(f"{output_dir}/ieee39_combined_data.csv", index=False)
...
np.save(f"{output_dir}/scaler_X_mean.npy", scaler_X.mean_)
np.save(f"{output_dir}/scaler_X_scale.npy", scaler_X.scale_)
np.save(f"{output_dir}/scaler_y_mean.npy", scaler_y.mean_)
np.save(f"{output_dir}/scaler_y_scale.npy", scaler_y.scale_)

# Save raw data
X_direct.to_csv(f"{output_dir}/X_direct.csv", index=False)
y_direct.to_csv(f"{output_dir}/y_direct.csv", index=False)

# Save scaled data
pd.DataFrame(X_scaled, columns=X_direct.columns).to_csv(f"{output_dir}/X_direct_scaled.csv", index=False)
pd.DataFrame(y_scaled, columns=y_direct.columns).to_csv(f"{output_dir}/y_direct_scaled.csv", index=False)

# Also save in numpy format
np.save(f"{output_dir}/X_direct.npy", X_direct.values)
np.save(f"{output_dir}/y_direct.npy", y_direct.values)
np.save(f"{output_dir}/X_direct_scaled.npy", X_scaled)
np.save(f"{output_dir}/y_direct_scaled.npy", y_scaled)
...
if 'class' in labels.columns:
    ...
    class_data.to_csv(f"{output_dir}/y_feasibility.csv", index=False)
    np.save(f"{output_dir}/y_feasibility.npy", class_data.values)
